...
function Notes() {
  ...
  const handleTranslation = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_SERVER_URL}/api/translate`,
        {
          text: text,
  ...
  const handleUpdateSavedText = () => {
    if (selectedText) {
      const updatedTexts = savedTexts.map((item) =>
        item.id === selectedText.id ? { ...item, content: text } : item
      );
      setSavedTexts(updatedTexts);
      localStorage.setItem("kahiniTexts", JSON.stringify(updatedTexts));
      setIsEditMode(false);
  ...
  return (
    <div className="flex bg-white dark:bg-gray-900 min-h-[calc(100vh-4rem)] mt-16 text-gray-800 dark:text-gray-200 font-sans transition-colors duration-300 p-4">
      {/* Editor Section */}
      <div className="w-3/4 p-3">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
          {/* Text Editor Toolbar */}
          <div className="flex bg-white dark:bg-gray-700 rounded-xl p-1 mb-4 gap-4 flex-wrap">
          ...
          <div
            ref={editorRef}
            className="w-full bg-transparent text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-xl p-4 outline-none text-base leading-relaxed min-h-80"
            contentEditable
            style={{ fontFamily: selectedFont }}
          ...
          <div className="mt-4 flex justify-end">
            <button
              className={`bg-gray-600 text-gray-200 border-none px-4 py-2 rounded-lg cursor-pointer flex items-center gap-2 transition-colors ${
                isListening ? "bg-green-700" : "hover:bg-gray-500"
              }`}
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex gap-2">
              {isEditMode ? (
                ...
                <button
                  onClick={handleSave}
                  className="bg-yellow-500 dark:bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 dark:hover:bg-yellow-700 transition-colors"
                >
                  <Save size={16} />
                </button>
              )}
          ...
          {/* Translation Section */}
          {showTranslation && (
            <div className="mt-4 border-t border-yellow-200 dark:border-yellow-900 pt-4">
              <div className="bg-yellow-100 dark:bg-gray-700 rounded-lg p-4">
                {isLoading ? (
      ...
      <div className="w-1/4 p-2 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 rounded-r-2xl">
        <h2 className="flex items-center gap-2 mb-4 text-yellow-600 dark:text-yellow-400 text-xl font-bold">
        ...
        {savedTexts.length === 0 ? (
          ...
          savedTexts.map((savedText) => (
            <div key={savedText.id} className="bg-gray-700 rounded-lg p-4 mb-4">
              <p className="mb-2 max-h-24 overflow-hidden text-ellipsis">
                {savedText.content}
              </p>
